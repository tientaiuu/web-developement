...
// Phần xử lý sự kiện click trên các nút + / – / xóa
async function cartEventHandler(e) {
  ...
  try {
    ...
    if (action === 'decrease') {
      const currentQty = cart[idx]?.quantity ?? 0;
      if (currentQty > 1) {
        await updateCartQuantity(bookId, currentQty - 1);
      } else {
        console.log('🛒 [Cart] Cannot decrease below 1');
...
async function updateCartQuantity(bookId, newQuantity) {
  ...
  try {
    console.log(`🛒 [Cart] Setting quantity for ${bookId} → ${newQuantity}`);

    if (newQuantity < 1) {
      // Nếu giảm xuống dưới 1 thì xoá luôn
      await window.ApiService.removeCartItem(bookId);
    } else {
      // Gọi API cập nhật số lượng tuyệt đối
      // Nếu API của bạn cần thêm tham số 'type', hãy truyền vào tương ứng
      await window.ApiService.updateCartItem(bookId, newQuantity);
    }

    // Tải lại giỏ để đồng bộ UI
    await loadCart();

  } catch (error) {
    console.error('❌ [Cart] Error in updateCartQuantity:', error);

    // Fallback đơn giản: tính sai lệch và thêm/xoá tương ứng
    const currentItem = cart.find(item => item.id === bookId);
    const currentQty  = currentItem?.quantity ?? 0;
    const diff        = newQuantity - currentQty;

    if (diff > 0) {
      await window.ApiService.addToCart(bookId, diff);
    } else if (diff < 0) {
      await window.ApiService.removeCartItem(bookId);
      if (newQuantity > 0) {
        await window.ApiService.addToCart(bookId, newQuantity);
      }
    }

    // Cuối cùng reload lại giỏ
    await loadCart();
  }
}
...
async function updateCartQuantity(bookId, newQuantity) {
  ...
  try {
    ...
    if (newQuantity < 1) {
      ...
      // chỉ gọi duy nhất updateCartItem với số lượng tuyệt đối
      await window.ApiService.updateCartItem(bookId, newQuantity);
    }
    // sau khi thay đổi xong thì load lại để đồng bộ
    await loadCart();
  } catch (error) {
    console.error('❌ [Cart] Error in updateCartQuantity:', error);
    // nếu thật sự cần fallback, phân biệt tăng/giảm:
    const currentItem = cart.find(item => item.id === bookId);
    ...
    if (diff > 0) {
      ...
      // nếu API không hỗ trợ trừ, xoá rồi thêm lại số lượng mới
      await window.ApiService.removeCartItem(bookId);
      if (newQuantity > 0) {
        await window.ApiService.addToCart(bookId, newQuantity);
      }
